...
public class AuthrityInterceptor extends HandlerInterceptorAdapter {
	...
	// 권한 정보를 등록하는 초기화 블록
	{
		...
		authMap.put("/message/update.do", 1);
	...
	throws Exception {
		...
		if(pageGradeNo != null) {
			...
			if(vo == null) {
				...
				// 로그인 폼이나 처리인 경우를 제외하고 전달되는 url과 전달되는 쿼리를 session에 넣어 놨다가 바로 가도록 처리한다.
				if(!uri.equals("/member/loginForm.do") && !uri.equals("/member/login.do"))
					session.setAttribute("goURI", uri + "?" + request.getQueryString());
