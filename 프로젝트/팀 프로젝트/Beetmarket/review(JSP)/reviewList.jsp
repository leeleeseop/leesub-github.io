...
$(function() {
 ...
 // 질문 수정
    ...
    // 답변 등록
    $(".reviewList").on("click",".answeran_write_Btn", function() {
        let goodsQNA = $(this).data("goodsqna");
        let answer = $(this).data("answer");

        $("#answeranwriteModal #GQ_answeranwriteModal").val(goodsQNA);
        $("#answeranwriteModal #answeranwrite_answer").val(answer);
    });
    
    
 // 모달 창의 답장 버튼에 이벤트 바인딩
    $(document).on("click", "#answeran_write_ModalBtn", function(){

        // 'goodsQNA' 변수 선언 및 초기화
        let goodsQNA = $("#GQ_answeranwriteModal").val();
        console.log("goodsQNA:", goodsQNA); // 디버깅용 로그
        ...
        // 'answer' 변수 선언 및 초기화
        let answer = $("#goodsWrite_answer").val();
        ...
        $.ajax({
            type: "POST",
            url: "/goods_qna/answeranwrite.do",
            data: JSON.stringify(goodsQna),
            contentType: "application/json",
            success: function(response){
            	alert(response);
                    $("#answeranwriteModal").modal("hide");
                    // 댓글 리스트를 다시 불러옵니다.
                    $(".reviewList").load("/goods_qna/list.do?goodsNo=" + ${param.goodsNo}+"&store_name="+"${vo.store_name}");
            },
            error: function(xhr, status, err){
   ...
   $(document).on("click", "#answeran_Delete_ModalBtn", function(){
	...
	$(document).ready(function(){
	    // 모달이 열릴 때 데이터를 채워넣음
	    $('#QnaModal').on('show.bs.modal', function (event) {
	        var button = $(event.relatedTarget); // 버튼 클릭으로부터 데이터 가져오기
	        var question = button.data('question');
	        var answer = button.data('answer');
	        var answerDate = button.data('answerdate');

	        // 모달 내의 요소에 데이터 삽입
	        var modal = $(this);
	        modal.find('#modalQuestion').text(question);

	        // 답변이 있는 경우에만 답변 영역을 보여줌
	        if (answer) {
	            modal.find('#modalAnswer').text(answer);
	            modal.find('#modalAnswerDate').text(answerDate);
	            modal.find('#answerSection').show(); // 답변 섹션 보이기
	        } else {
	            modal.find('#answerSection').hide(); // 답변 섹션 숨기기
	        }
	    });
	});
});
