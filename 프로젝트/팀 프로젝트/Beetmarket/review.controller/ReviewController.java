...
public class ReviewController {
	...
	public String list(Model model, HttpServletRequest request, Long goodsNo,Double scroll) throws Exception {
	    ...
	    model.addAttribute("totalReviewCount", totalReviewCount);	// 리뷰 총 개수를 표시
	    model.addAttribute("averageRating", averageRating);			// 평균평점을 표시
	    log.info(pageObject);							
	    model.addAttribute("pageObject", pageObject);				// 페이지번호 등 전달
	    model.addAttribute("scroll", scroll);						// 스크롤 상태 유지
	...
	public String replywrite(ReviewVO vo, HttpServletRequest request, 
		...
		// 처리 결과에 대한 메시지 처리
		rttr.addFlashAttribute("답변이 등록되었습니다");
		return "redirect:/goods/view.do?goodsNo=" + vo.getGoodsNo() + "&inc=1";
	}
