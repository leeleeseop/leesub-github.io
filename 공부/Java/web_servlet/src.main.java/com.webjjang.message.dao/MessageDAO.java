...
public class MessageDAO extends DAO {
	...
	public Long getTotalRow(PageObject pageObject) throws Exception {
		...
		try {
			...
			// 3. sql - 아래 LIST
			System.out.println(
					"MessageDAO.getTotalRow().sql=" + TOTALROW + getModeSql(pageObject) + getSearch(pageObject));
			// 4. 실행 객체 & 데이터 세팅
	...
	final String LIST = "" + " select  no, senderId, senderName, senderPhoto, sendDate, "
			+ "	accepterId, accepterName, accepterPhoto, acceptDate " + " from ( " + " select rownum rnum, no, "
			+ " senderId, senderName, senderPhoto, sendDate, " + " accepterId, accepterName, accepterPhoto, acceptDate "
			+ " from ( " + " select m.no, " + " m.senderId, sm.name senderName, sm.photo senderPhoto, "
			+ " to_char(m.sendDate, 'yyyy-mm-dd') sendDate, "
			+ " m.accepterId, am.name accepterName, am.photo accepterPhoto, "
			+ " to_char(m.acceptDate, 'yyyy-mm-dd') acceptDate " + " from message m, member sm, member am ";
	...
	final String VIEW = "" + " select m.no, m.content, " + " m.senderId, sm.name senderName, sm.photo senderPhoto, "
			+ " to_char(m.sendDate, 'yyyy-mm-dd') sendDate, "
			+ " m.accepterId, am.name accepterName, am.photo accepterPhoto, "
			+ " to_char(m.acceptDate, 'yyyy-mm-dd') acceptDate " + " from message m, member sm, member am "
